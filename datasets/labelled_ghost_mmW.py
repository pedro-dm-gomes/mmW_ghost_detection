    ...
    def __init__(
        ...
        if train:
            ...
            for run in h5_files:
                ...
                # Manipulate labels
                label_id = new_radar_data[:,:,3]
                print("label_id.shape",label_id.shape)
                
                #print("new_radar_data[0,0]", new_radar_data[5,50:40])
                
                #for frame in range (0,new_radar_data.shape[0]):
                    #for point in range(0, new_radar_data.shape[1]):

                        #if(new_radar_data[frame,point,3] != -1 and new_radar_data[frame,point,3] != -2 ):
                        #    new_radar_data[frame,point,3] = 0 #OBJECT
                        #if(new_radar_data[frame,point,3] ==-2): 
                        #    new_radar_data[frame,point,3] = 1 # NOISE
                        # -1 = IGNORE 0- OBJECT # 1  NOISE

                last_column = new_radar_data[:, :, -1]
